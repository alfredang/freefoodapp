...
- **`FoodListingStore`** (`Services/`) — `@MainActor ObservableObject`, the single source of truth for listings, backed by the **CloudKit public database** (`publicCloudDatabase`). Two things to know:
  ...
  - **Expiry is hide-first, delete-best-effort.** Expired listings (past their end time, or >7 days old) are *hidden* client-side by the `isActive` filter in `rebuild()` — this is the guarantee, and it means **users never see a stale post regardless of what's in the cloud**. *Deleting* them is separate and best-effort: CloudKit's public-DB security roles let a device delete only records **its own user created** (the `_creator` role), so `purgeStaleFromCloud()` sweeps your own old posts but silently no-ops on strangers'. Expired records created by other users therefore accumulate in CloudKit indefinitely, invisible to everyone. That is accepted, not a bug.

  ⚠️ **Do not "fix" this with a server-side cleanup job.** It was tried (removed 2026-07-14) and doesn't work without loosening production security: a CloudKit server-to-server key is not tied to an iCloud user, so it evaluates as the `_world` role and gets `ACCESS_DENIED — WRITE operation not permitted` on delete. Making it work means granting `_world` Write on `FoodListing`, which lets any API-token holder delete *live* listings. Not worth it to reclaim invisible rows. (If you ever do revisit it: the REST delete must use `operationType: "forceDelete"` — plain `"delete"` needs each record's `recordChangeTag`, which a query response doesn't carry.)
